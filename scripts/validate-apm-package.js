...
if (!fs.existsSync(apmYml)) err("Missing apm.yml at repo root");
else ok("apm.yml exists");

if (!fs.existsSync(agentMd))
  err(
    `Missing .apm agent markdown (checked: ${agentAgentMd} or ${agentLegacyMd})`,
  );
...
if (fs.existsSync(agentJson)) {
  try {
    ...
    if (missing.length)
      err("agent sidecar JSON missing fields: " + missing.join(", "));
    else ok("agent sidecar JSON has required fields");
  } catch (e) {
    err("Failed to parse agent sidecar JSON: " + e.message);
  }
} else {
  ...
  if (fs.existsSync(agentMd)) {
    try {
      ...
      if (!m) {
        err("No YAML frontmatter found in " + path.basename(agentMd));
      } else {
        const fm = m[1];
        const hasDescription = /(^|\r?\n)\s*description\s*:\s*.+/i.test(fm);
        const hasTemp = /(^|\r?\n)\s*temperature\s*:\s*.+/i.test(fm);
        if (!hasDescription) err("YAML frontmatter missing description");
        else ok("agent frontmatter contains description");
        if (!hasTemp) err("YAML frontmatter missing temperature");
        else ok("agent frontmatter contains temperature");
      }
    } catch (e) {
