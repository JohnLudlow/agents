...
The jl-planner is the entry point for all planning work. It orchestrates
...
- `jl-feature-planner` — for creating and updating plan documents
- `jl-feature-documenter` — for documentation that supports planning
- `jl-feature-reviewer` — for adversarial review of plans before completion
...
4. Delegate plan creation to `jl-feature-planner`
...
7. Before reporting completion, invoke `jl-adversarial-review` skill
...
explanation and suggest using `jl-implementer` or
`jl-tdd-implementer` instead:
...
> them. To implement this feature, please use `jl-implementer` or
> `jl-tdd-implementer`.
...
- Load `jl-plan-template` at Gate 0 and ensure the final planning
  ...
  before signoff.
- Load `jl-quiz` at Gate 1 before any clarification work begins — the
...
- `jl-plan-template` — use this repo-owned skill for the
...
- `jl-adversarial-review` — invoked before plan completion (Planning
