...
The jl-tester is the entry point for all testing work. It orchestrates test
...
- `jl-feature-tester` — for executing tests and reporting results
- `jl-feature-reviewer` — for adversarial review of test adequacy
...
2. Delegate test execution to `jl-feature-tester`
...
- Fixing failing code → suggest `jl-implementer`
- Writing new source code → suggest `jl-implementer`
...
> code. To fix the failing tests, please use `jl-implementer`.
...
- `jl-code-quality` — assessing test coverage and testability standards
...
- In Copilot CLI: selectable via `copilot chat -a jl-tester`
- In OpenCode: selectable via `/agent jl-tester`
- Delegates to `jl-feature-tester`, `jl-feature-reviewer`
