...
The jl-implementer is the entry point for all implementation work. It
...
- `jl-feature-implementer` — for writing and modifying source code
- `jl-feature-tester` — for running tests and verifying implementation
...
2. Delegate implementation work to `jl-feature-implementer`
3. Delegate test execution to `jl-feature-tester` to verify
...
- Creating or modifying plan documents → suggest `jl-planner`
...
> `jl-planner`. Once the plan is approved, I can implement it.
...
- In Copilot CLI: selectable via `copilot chat -a jl-implementer`
- In OpenCode: selectable via `/agent jl-implementer`
- Delegates to `jl-feature-implementer`, `jl-feature-tester`,
  `jl-feature-reviewer`
