...
- Your agent or the user needs to spawn a subagent (e.g., jl-feature-planner)
...
only agents. When you try `/fleet /jl-quiz ...`, it fails because quiz
...
1. User: `/jl-planner` (top-level agent)
2. Agent launches `jl-feature-planner` as background subagent (via
   `task` tool)
3. Subagent completes planning artifact
4. Agent invokes `jl-adversarial-review` skill (no fleet needed)
5. Returns to user with artifact + review verdict

**Key:** The `task` tool (`task: { agent_type: "jl-feature-planner" }`)
...
4. Agent calls `jl-adversarial-review` skill for reviews
...
3. Agent offers: "Subagent spawning unavailable in browser. Review by
   jl-feature-reviewer agent?" (user choice)
4. User confirms
5. Agent invokes `jl-feature-reviewer` agent as fallback
...
- If subagent spawning is critical, keep `jl-feature-reviewer` agent
  as fallback
...
- Does skill invocation work? (check `disable-model-invocation: false` on
  jl-adversarial-review)
- If neither: use jl-feature-reviewer agent as fallback
...
See **[DEPENDENCIES.md](references/DEPENDENCIES.md)** for relationships to jl-planner,
jl-feature-planner, jl-adversarial-review, jl-feature-reviewer,
and jl-planning-workflow.
