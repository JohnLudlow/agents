---
name: jl-plan-template
description: "Provides the canonical plan document template and structure for feature plans stored in docs/plans/. Use when creating or validating a plan document with jl-feature-planner or jl-planner."
---
...
produced by `jl-feature-planner` and `jl-planner`. It exists
...
4. Keep the **Plan Target and Governance** section accurate — it records
   ...
   `jl-issue-management` skill for the vocabulary behind these
   fields.
...
- Use `jl-issue-management` to decide the plan target and provider
  before filling in the **Plan Target and Governance** section.
- Use `jl-quiz` before drafting the plan when intent or scope is
  still fuzzy — a plan built on unresolved decisions will need rework.
- Use `jl-markdown-standards` for general markdown formatting rules
