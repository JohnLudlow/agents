...
- **johnludlow-issue-management** — Use when the resolved objective needs a
  home in a provider-native system (GitHub issue, Azure DevOps work item).
  This skill only surfaces and resolves decisions; johnludlow-issue-management
  decides where resulting artifacts live.
- **johnludlow-markdown-standards** — Use for the structure and formatting of
  any questionnaire document produced by this skill.
- **johnludlow-planning-workflow** — This skill (quiz) is invoked during BLOCKER
  1 (Shared Understanding) in planning agents. The planning workflow skill
  orchestrates all BLOCKERs.
- **johnludlow-subagent-spawning** — Reference this skill if you're implementing
...
```text
Planning Agent (johnludlow-planner)
  ↓
  BLOCKER 0: Preference Resolution (in this skill)
  ↓
  BLOCKER 1: Shared Understanding
    ↓
    Invoke johnludlow-quiz (this skill, Mode A or B)
    ↓
    Returns: objective + resolved decisions
  ↓
  BLOCKER 2: Plan Target Selection (consult johnludlow-issue-management)
  ↓
  BLOCKER 3: Issue Workflow (consult johnludlow-issue-management)
  ↓
  Planning Steps (produce artifact)
  ↓
  Invoke johnludlow-adversarial-review (before completion)
```
